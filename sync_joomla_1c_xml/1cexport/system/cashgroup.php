...
function inserCashgroup($xml) 
{
	...
	$cash_group->XML($xml);
	//$cash_group = simplexml_load_string($xml->readOuterXML());
	...
	//$log->addEntry ( array ('comment' => '$cash_group->name'.strval($xml) ) );
	//$log->addEntry ( array ('comment' => '$cash_group->name'.strval($cash_group) ) );
	while($cash_group->read()) 
	{
		
		//$log->addEntry ( array ('comment' => '$cash_group->name'.$cash_group->name ) );
		if($cash_group->nodeType == XMLReader::ELEMENT ) 
		{
			...
			switch($cash_group->name) 
			{
				...
				case 'КурсВалюты':
					...
					$exchange_rage = round(1/$val_rate,12);
					$data['rate'] = $exchange_rage;
					//$log->addEntry ( array ('comment' => '$val_rate, $exchange_rage'.$val_rate.', '.$exchange_rage ) );

					break;					
				case 'Налог':
										
					$xml_nds = simplexml_load_string($cash_group->readOuterXML());
					$data['nds'] = trim($xml_nds->УчтеноВСумме);	
					$data['nds_name'] = trim($xml_nds->Наименование);
					unset($xml_nds);
					$cash_group->next();
					break;
