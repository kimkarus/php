...
function inserCategory($xml, $parent = 0) 
{
	...
	{		
		...
		{ 
			...
			$image = ( string ) $category->Картинка;
				...
				if(VM_VERVM == '1')
				{
					...
					$ins->category_full_image = '';
						...
						if(substr ( $data['image'], -4 ) == 'jpeg'){
							$ins->category_thumb_image = str_replace(".jpeg", "", $image);;
							$ins->category_full_image = "resized".DS.$tbn_img."_".VM_TBN_H."x".VM_TBN_W.".".VM_JPG_S;;
						}
					}
					else
					{
						$ins->category_thumb_image = '';
						$ins->category_full_image = $image;
					}*/
					$ins->list_order = 1;
				}
				elseif(VM_VERVM == '2')
